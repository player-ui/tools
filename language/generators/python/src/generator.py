...
import ast
from typing import Any, List, Dict, Literal, NamedTuple, Optional, Union
from pathlib import Path
from copy import deepcopy

from player_tools_xlr_types.nodes import (
    AndType, NamedType, ObjectProperty, ObjectType, NodeType, OrType, RefType, is_and_type, is_any_type, is_named_type_with_generics, is_null_type,
    is_object_type, is_array_type, is_primitive_const, is_record_type, is_string_type, is_number_type, 
    is_boolean_type, is_named_type, is_or_type, is_ref_type, is_undefined_type, is_unknown_type
)
...
def generate_python_classes(named_object_type: NamedType[ObjectType], type: Literal['asset', 'view'], output_dir: str = ".") -> str:
    """
...
class ClassGenerator:
    ...
    def __init__(self, named_object_type: NamedType[ObjectType], output_dir: str, type: Literal['asset', 'view']):
        ...
        self.generic_tokens = dict((obj.symbol, obj) for obj in named_object_type.genericTokens) if is_named_type_with_generics(named_object_type) else dict()
    ...
    def _collect_nested_objects(self, node: Union[NodeType, NamedType], parent_prop: Optional[str]) -> None:
    ...
    def _collect_from_object_type(self, node: ObjectType, parent_prop: str) -> None:
        ...
        if is_named_type_with_generics(node):
            for generic_token in node.genericTokens:
                ...
                if (not is_ref_type(token) and is_object_type(token) and 
                    symbol not in self.classes_to_generate.keys()):
        ...
        if is_named_type(node):
            ...
            class_name = (self._generate_class_name(node.title.split(".")[-1]) if node.title else parent_prop).title()
            if class_name not in self.classes:
                self.classes.append(class_name)
                self.classes_to_generate[class_name] = node
    ...
    def _create_super_call(self, is_asset: bool) -> ast.Expr:
        ...
        if is_asset:
            args: List[ast.expr] = [ast.Name(id='id', ctx=ast.Load()), ast.Name(id='self.type', ctx=ast.Load())]
        else:
            args = []
    ...
    def _add_imports(self, module: ast.Module) -> None:
        ...
        imports = [
            ...
            ast.ImportFrom(
                module= '{}.view'.format(PLAYER_DSL_PACKAGE),
                names=[ast.alias(name='Asset', asname=None)],
                level=0
            ),
            # from lang.utils.serialize import Serializable
            ast.ImportFrom(
                module='{}.utils.serialize'.format(PLAYER_DSL_PACKAGE),
    ...
    #TODO merge with _generate_nested_class
    def _generate_main_class(self) -> ast.ClassDef:
        ...
        extends_name = "Asset" if any(key == "validation" for key in object_type.properties.keys()) else self.type 
        ...
        if(object_type.extends):
            ...
            if is_ref_type(extended_node) and extended_node.ref.startswith("Asset") and extended_node.genericArguments and len(extended_node.genericArguments) == 1:
        ...
        with_methods = self._generate_with_methods(object_type, is_asset=True)
    ...
    def _generate_nested_class(self, class_name: str, object_type: ObjectType) -> ast.ClassDef:
        ...
        # Add with* methods (getters/setters)
        with_methods = self._generate_with_methods(object_type, is_asset=False)
        class_def.body.extend(with_methods)
        return class_def
    ...
    def _add_property_annotations(self, class_def: ast.ClassDef, object_type: ObjectType) -> None:
        ...
        if new_names:
            map_arg = ast.Assign(targets=[ast.Name(id="_propMap", ctx=ast.Store())], value=ast.Dict(keys=list(new_names), values=list(original_names)))
    ...
    def _generate_with_methods(self, object_type: ObjectType, is_asset: bool) -> list[ast.FunctionDef]:
    ...
    def _generate_simple_with_method(self, method_name: str, prop_info: PropertyInfo) -> ast.FunctionDef:
        ...
        method_def = ast.FunctionDef(
            ...
            body=[
                ...
                ast.Assign(
                    targets=[ast.Attribute(value=COMMON_AST_NODES['self'], attr=prop_info.clean_name, ctx=ast.Store())],
    ...
    def _generate_asset_with_method(self, method_name: str, prop_info: PropertyInfo) -> ast.FunctionDef:
        """Generate a with* method for Asset properties using _withSlot."""
        is_array_of_assets = is_array_type(prop_info.node)
        
        is_asset_wrapper = prop_info.node.ref.startswith("AssetWrapper") if is_ref_type(prop_info.node) else False
        body = [
            ast.Expr(
                value=ast.Call(
                    func=ast.Attribute(value=COMMON_AST_NODES['self'], attr='_withSlot', ctx=ast.Load()),
                    args=[
    ...
    def _generate_array_with_methods(self, method_name: str, prop_info: PropertyInfo) -> list[ast.FunctionDef]:
        """Generate with* methods for array properties (set and append)."""
        methods = []
        
        # Get element type for append method
        element_type = (self._convert_xlr_to_ast(prop_info.node.elementType, f"{prop_info.clean_name}")
                       if is_array_type(prop_info.node) else COMMON_AST_NODES['Any'])
    ...
    def _create_array_append_body(self, prop_info: PropertyInfo) -> list[ast.stmt]:
        ...
        return [
            ...
            ast.If(
                test=ast.Compare(
                    left=ast.Attribute(value=COMMON_AST_NODES['self'], attr=prop_info.clean_name, ctx=ast.Load()),
                    ops=[ast.Is()],
            ...
            ast.Expr(
                value=ast.Call(
                    func=ast.Attribute(
                        value=ast.Attribute(value=COMMON_AST_NODES['self'], attr=prop_info.clean_name, ctx=ast.Load()),
                        attr='append',
                        ctx=ast.Load()
                    ),
    ...
    def _handle_or_type(self, node: OrType, prop_name: str) -> ast.expr:
        ...
        if all(is_primitive_const(t) for t in node.or_types):
            ...
            union_types: List[ast.expr] = [ast.Constant(value=or_type.const) for or_type in node.or_types] # type: ignore
            ...
            # Handle Union types
            union_types = [self._convert_xlr_to_ast(or_type, prop_name) for or_type in node.or_types]
    ...
    def _handle_and_type(self, node: AndType, prop_name: str) -> ast.expr:
        ...
        # For other cases, fall back to Union (Python doesn't have native intersection types)
            ...
            intersection_types = [self._convert_xlr_to_ast(and_type, prop_name) for and_type in and_types]
    ...
    def _merge_object_types(self, object_types: List[NodeType], prop_name: str, name: Optional[str] = "") -> ast.expr:
        ...
        merged_class_name = name if name else self._generate_merged_class_name(prop_name, object_types)
    ...
    def _handle_intersection_with_unions(self, and_types: List[NodeType], prop_name: str) -> ast.expr:
        ...
        for combination in current_combinations:
            ...
            if all(is_object_type(t) for t in intersection_candidate):
                result_types.append(self._merge_object_types(intersection_candidate, f"{prop_name}_intersection"))
            else:
