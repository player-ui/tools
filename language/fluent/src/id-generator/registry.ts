...
export class IDRegistry {
  ...
  /**
   * Generates an ID without registering it as used.
   * This is useful for intermediate ID generation (e.g., when generating parent IDs
   * for nested context creation) where you don't want to consume the ID.
   *
   * @param baseId - The desired ID
   * @returns The ID (unmodified, without collision detection)
   */
  peek(baseId: string): string {
    // Return the ID as-is without any collision detection or registration
    return baseId;
  }
