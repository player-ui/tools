...
describe('toJSON function', () => {
  ...
  test('converts object nodes to JSON', () => {
    ...
    // Object with props
    const propsObj = new Map<string, JsonType>([['key1', 'value1'], ['key2', 42]]);
    expect(toJSON(createObjectNode(propsObj))).toEqual({ key1: 'value1', key2: 42 });
...
describe('Circular reference handling', () => {
  test('maintains circular references in objects and arrays', () => {
    ...
    const objResult = toJSON(createValueNode(circularObj)) as Record<string, any>;
    ...
    parent.child = child;
    const complexResult = toJSON(createValueNode(parent)) as Record<string, any>;
