...
test("returns correct type name", () => {
  ...
  const expander = new RequiredExpander(typeAnalyzer);

  expect(expander.getTypeName()).toBe("Required");
});
...
test("validates type arguments count", () => {
  ...
  expect(consoleWarnSpy).toHaveBeenCalledWith(
    "Required expects 1 type arguments, got 2",
  );

  consoleWarnSpy.mockRestore();
});

test("expands required type for interface reference", () => {
  const project = createMockProject();
  ...
  const typeNode = createTypeNode(project, "PartialUser");
  ...
  expect(result?.name).toBe("requiredUser");
  expect(result?.typeAsString).toBe("Required<PartialUser>");
...
test("makes nested object properties required", () => {
  const project = createMockProject();
  setupTestInterface(project);
  const typeAnalyzer = new TypeAnalyzer();
  const expander = new RequiredExpander(typeAnalyzer);
  ...
  expect(profileProperty.isOptional).toBe(false);
  expect(profileProperty.type).toBe("object");

  // Nested properties should also be required
...
test("warns for unknown interface", () => {
  ...
  expect(result).toBe(null);
  expect(consoleWarnSpy).toHaveBeenCalledWith(
    "Could not find interface for Required source type: UnknownInterface",
  );
...
test("handles circular references", () => {
  ...
  const typeNode = createTypeNode(project, "OptionalNode");
...
test("expands required with direct type analysis", () => {
  const project = createMockProject();
  const typeAnalyzer = new TypeAnalyzer();
...
test("preserves non-object properties as required", () => {
  const project = createMockProject();
  project.createSourceFile(
    "/mixed.ts",
    `
  ...
  const typeNode = createTypeNode(project, "MixedProps");
