...
- [Generating Custom Builders](#generating-custom-builders)
  ...
  - [Generated Builder Features](#generated-builder-features)
...
The Fluent DSL includes a code generator (`@player-tools/fluent-generator`) that creates type-safe builders from your TypeScript asset types.

#### CLI Usage

For projects with XLR type definitions:
...
For TypeScript-based generation (recommended for most projects):
...
// Create TypeScript program from your type files
...
// Generate builder for each asset type
for (const assetType of result.data.types) {
  const config: GeneratorConfig = {
    fluentImportPath: "@player-tools/fluent",
    typeImportPathGenerator: (typeName) => `./types/${typeName}.js`,
  };

  const code = generateFluentBuilder(assetType, outputDir, config);
  writeFileSync(`${outputDir}/${typeName}.builder.ts`, code);
}
```

See `scripts/generate-mocks.ts` for a complete working example.
...
The generator automatically enhances your builders with:

**AssetWrapper Handling** — Nested assets are automatically wrapped, so you can pass `text()` directly instead of wrapping it in `{ asset: ... }`.

**TaggedTemplateValue Support** — Primitive types (string, number, boolean) accept both static values and dynamic bindings/expressions via tagged templates.
...
When using the programmatic API, configure the generator with `GeneratorConfig`:
...
interface GeneratorConfig {
  ...
  /** Function to generate import paths for referenced types */
